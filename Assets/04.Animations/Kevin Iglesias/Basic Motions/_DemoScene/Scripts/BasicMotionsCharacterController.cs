...
namespace KevinIglesias
{
    ...
    public class BasicMotionsCharacterController : MonoBehaviour
    {
        ...
        [Header("[FINISH LINE]")]
        public BoxCollider finishLineArea; //COLLIDER DELIMITING CIRCUIT ENDING ZONE
        ...
        ///SCAN FINISH AREA COLLIDER AND DETECT IF PLAYER IS INSIDE
        private void CheckFinishArea()
        {
            if(finishLineArea.bounds.Contains(transform.position)) //CHECK IF CHARACTER IS INSIDE FINISH AREA COLLIDER BOUNDS
            {
                if(blockControls == false) //CHECK TO ENSURE THIS HAPPENS ONLY ONCE (WHILE PLAYER CAN CONTROL CHARACTER)
                {
                    blockControls = true; //TEMPORARY DISABLE PLAYER CONTROLS
                    
                    //VICTORY ANIMATION
                    animator[0].SetBool("Victory", true);
                    animator[1].SetBool("Victory", true);
                    
                    finishLineArea.gameObject.SetActive(false); //DISABLE AREA TO GIVE PLAYER CHANCE TO MOVE AWAY
                    Invoke("AllowControls", 1f); //ALLOW AGAIN CONTROLS WHEN AFTER THE VICTORY ANIMATION FINISHES
                }
            }
        }
        private void AllowControls()
        {
            blockControls = false; //DISABLE PLAYER CONTROL LOCK
            Invoke("ResetFinishArea", 2f); //TIME MARGIN FOR CHARACTER TO CELEBRATE AGAIN
        }
        private void ResetFinishArea()
        {
            finishLineArea.gameObject.SetActive(true); //ENABLE FINISH AREA AGAIN
        }
        ...
        private bool IsCeilingAbove()
        {
            ...
            foreach(RaycastHit hit in hits)
            {
                ...
                if(hit.transform.parent == collisionsRoot)
                {
        ...
        private bool CheckGround()
        {
            ...
            for(int i = 0; i < groundRayOrigin.Length; i++)
            {
                ...
                if(closestHit != null) //OBSTACLE DETECTED
                {
                    ...
                    groundHitDetected = true;
                    Debug.DrawRay(aroundOrigin, Vector3.down * distanceToGround, Color.red);
                }else{
                    Debug.DrawRay(aroundOrigin, Vector3.down * distanceToGround, Color.green);
                }
